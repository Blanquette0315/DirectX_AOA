...
int CDevice::CreateSampler()
{
	...
	// Sampler의 Address_Mode의 WRAP은 UV값이 UV범위(0~1)를 벗어나면, 정수를 버리고 소수점으로 UV값을 다시 준다.
	// 즉, 1.2로 UV.x값이 벗어나면 WRAP에 의해 UV.x가 0.2로 변환되어 적용된다.
...
{
	float arrColor[4] = { 0.137f,0.082f,0.f,1.f };
