...
CRightWaponSlotScript::CRightWaponSlotScript()
	: CSlotScript(RIGHTWAPONSLOTSCRIPT)
{
	// 해당 슬롯이 무기 슬롯임을 알린다.
	m_bWaponSlot = true;
}
...
void CRightWaponSlotScript::begin()
{
	CSlotScript::begin();
}

void CRightWaponSlotScript::tick()
{
	CSlotScript::tick();
}

void CRightWaponSlotScript::CreateDefaultItemCard()
{
	Ptr<CPrefab> pWaponPrefab = nullptr;

	CPlayerScript* pPlayerScript = m_pUIMgrScript->GetGameMgrScript()->GetPlayerScript();

	tPlayer_Wapon WaponInfo = pPlayerScript->GetRightWaponInfo();
	...
	pPlayerScript->SetRightWaponType(WaponInfo.eWaponType);
}

void CRightWaponSlotScript::MoveItem()
{	
...
void CRightWaponSlotScript::ChangeInfo(tItem_Info _Info)
{
	if (_Info.eItemType == ITEM_TYPE::WAPON)
	{
		m_pUIMgrScript->GetGameMgrScript()->GetPlayerScript()->SetRightWaponInfo(_Info.tWapon);
	}
}

void CRightWaponSlotScript::MouseCheck()
{
